...
# Remove métricas existentes se já registradas (evita duplicação)
for collector in list(REGISTRY._collector_to_names.keys()):
    try:
        if hasattr(collector, '_name'):
            if collector._name in ['http_request_duration_seconds', 'http_requests_total']:
...
def run_api_server():
    ...
    # Executar servidor com a app global que já tem /health e /metrics
    uvicorn.run(
