...
# -*- coding: utf-8 -*-
"""
Slot Context - Sistema de rastreamento de slots em cascata
...
def bind_slot(headers: dict, defaults: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Vincula informações de slot do request/contexto atual
    
    Args:
        headers: Headers HTTP do request
    ...
    slot_data = {
        "slot_id": headers.get("x-slot-id") or d.get("slot_id") or uuid.uuid4().hex[:12],
        "slot_parent": headers.get("x-slot-parent") or d.get("slot_parent") or "",
        "slot_root": headers.get("x-slot-root") or d.get("slot_root") or "",
        "slot_chain": headers.get("x-slot-chain") or d.get("slot_chain") or "",
        "slot_stage": headers.get("x-slot-stage") or d.get("slot_stage") or "decide",
        "slot_strategy": headers.get("x-slot-strategy") or d.get("slot_strategy") or "unknown",
        "slot_instance": headers.get("x-slot-instance") or d.get("slot_instance") or "ai-gateway-1",
        "slot_tenant": headers.get("x-slot-tenant") or d.get("slot_tenant") or "default",
    }
...
def labels_for_metrics() -> Dict[str, str]:
    ...
    return {
        "slot_stage": s.get("slot_stage", "decide"),
        "slot_strategy": s.get("slot_strategy", "unknown"), 
        "slot_instance": s.get("slot_instance", "ai-gateway-1"),
    }
...
def create_child_slot(stage: str, strategy: Optional[str] = None) -> Dict[str, Any]:
    ...
    child_data = {
        ...
        "slot_instance": parent.get("slot_instance", "ai-gateway-1"),
...
def propagate_slot_headers() -> Dict[str, str]:
    """
    Gera headers para propagação do slot para outros serviços
    
    Returns:
        Dict com headers x-slot-* para propagação
    """
    s = slot_ctx.get({})
