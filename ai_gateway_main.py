...
TURBINADA: uvloop para performance otimizada (2-4x faster event loop)
"""
...
# TURBINADA: Instalar uvloop para event loop otimizado
try:
    import uvloop
    uvloop.install()
except ImportError:
    pass  # Fallback para asyncio padrão se uvloop não disponível
...
# Slot Context System - absolute import
from ai_gateway.slot_context import bind_slot, labels_for_metrics
...
# Métricas Prometheus slot-aware com registro seguro
# Métricas Prometheus slot-aware com registro seguro usando registry separado
...
@app.get("/health")
def health():
    """Health check endpoint - retorna OK"""
...
def run_api_server():
    ...
    # TURBINADA: Executar com uvloop (se disponível) + workers otimizados
    uvicorn.run(
        "ai_gateway.__main__:app",
        host=host,
        port=port,
        reload=debug,
        log_level="warning",  # TURBINADA: Reduz I/O de logs
        loop="uvloop",  # TURBINADA: Força uvloop se disponível
        workers=1,  # Single worker para ambiente Docker
        timeout_keep_alive=60  # TURBINADA: Keepalive otimizado
    )
